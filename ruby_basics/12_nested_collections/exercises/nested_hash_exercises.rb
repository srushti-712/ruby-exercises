def find_language_information(languages, language_name)
  ...
  return languages.dig(language_name)
end
...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  lang_info = languages.dig(language_name)
  lang_info[info_key] = info_value
  languages
end
...
def find_beautiful_languages(languages)
  ...
  beautiful_langs = {}
  languages.each do |lang, lang_info|
    lang_info.each do |key, value|
      if key == :is_beautiful? and value == true
        beautiful_langs[lang] = lang_info
        break
      end
    end
  end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  lang_info = languages[language_name]
  return nil unless lang_info
  facts = lang_info.select {|k, v| k == :facts }
